...
# 計算規則
# group by 新舊客戶
# 線下 : 銷單來源 = 非Shopline訂單 、 類別名稱 = 會員 、 會員分類 != 非會員
# 廣告 : 銷單來源 = Shopline訂單 、 類別名稱 = 會員 、 會員分類 != 非會員
...
def sum_purchase(Transaction_df):
    """
    # 計算規則 group by 新舊客戶
    # 線下 : 來源單號 = 非Shopline訂單 、 類別名稱 = 會員 、 會員分類 != 非會員
    # 廣告 : 來源單號 = Shopline訂單 、 類別名稱 = 會員 、 會員分類 != 非會員
    # KOL : 類別名稱 = 快樂益生 、 會員分類 != 非會員
    """
    # ### 客戶來的次數by年+月 
    ...
    #抓人次 by月份
    purchase_times = Transaction_df.groupby(['客戶廠商編號','日期年加月','來源單號','類別名稱']).客戶廠商編號.nunique()
    ...
    # ### 客戶帶來的總額與件數by年+月
    purchase_sum = Transaction_df.groupby(['客戶廠商編號','日期年加月','來源單號','類別名稱']).agg({'金額': sum}) #'客戶代號':len
    purchase_sum = purchase_sum.reset_index()
    #### 合併總額數量與次數
    purchase_detail = purchase_sum.merge(purchase_times,on=['客戶廠商編號','日期年加月','來源單號','類別名稱'],how='inner')
    #篩選金額>0
    ...
    purchase_detail = purchase_detail[['客戶廠商編號','日期年加月','月份新舊客戶','來源單號','類別名稱','人次']]
    return purchase_detail

def groupby_newold(purchase_detail,year_date):
    #線下#根據條件篩選
    offline_df = purchase_detail.query('來源單號 == "非Shopline訂單" & 類別名稱=="會員" ').groupby(["日期年加月",'月份新舊客戶'])["人次"].sum().reset_index()
    #廣告
    ad_df = purchase_detail.query('來源單號 == "Shopline訂單" & 類別名稱=="會員" ').groupby(["日期年加月",'月份新舊客戶'])["人次"].sum().reset_index()
    #KOL
    kol_df = purchase_detail.query('類別名稱=="快樂益生" ').groupby(["日期年加月",'月份新舊客戶'])["人次"].sum().reset_index()
    ...
    new_final_df.rename(columns = {'人次_x':'線下','人次_y':'廣告','人次':'KOL'}, inplace = True)
